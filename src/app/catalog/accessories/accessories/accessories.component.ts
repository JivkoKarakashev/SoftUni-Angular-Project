...
export class AccessoriesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const categoryUrls = this.accessoriesService.getCategoryUrls();
  ...
  onAddToCart(i: number): void {
    ...
    const { _ownerId, _id, _createdOn, image, altImages, cat, subCat, description, brand, size, color, quantity, price } = this.filteredItems[i];
  ...
  private deleteItem(i: number): Observable<Array<Item[]>> {
    this.loading = true;
  ...
  private fetchCatalogData(): Observable<Array<Item[]>> {
    this.loading = true;
    return this.accessoriesService.getCollectionComposition()
      .pipe(
        catchError(err => {
          this.loading = false;
          this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
        ...
        switchMap(compArr => {
          const categorySize = compArr.reduce((acc, curr) => acc += curr, 0);
          if (compArr.length === 0 || categorySize === 0) {
            this.loading = false;
            return EMPTY;
          }
          ...
          ///////////////////////////////////////
          return this.accessoriesService.getAccessoriesByPage([...this.paginationConfig.subcategoryConfigs])
            .pipe(
              catchError(err => {
                this.loading = false;
                this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
              ...
              tap(itmsArr => {
                ...
                const flattenedListItems = itmsArr.flat(1);
                this.cartItms = [...this.cartStateMgmnt.getCartItems()];
                this.cartItemsCounter = this.cartItms.length;
                this.listItems = this.checkForInCartAlready.check(itmsArr, this.cartItms);
                this.filteredItems = [...this.listItems];
                this.setAnimationStates();
                this.sizeFilterOptions = Array.from(new Set(flattenedListItems.map(itm => itm.size).flat(1).sort((a, b) => {
                ...
                this.priceFilterOptions = Array.from(new Set(flattenedListItems.map((itm, i) => (i !== itmsArr.length - 1) ? Math.trunc(itm.price) : Math.ceil(itm.price)).sort((a, b) => a - b)));
