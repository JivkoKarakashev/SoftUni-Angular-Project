...
export class WatchesComponent implements OnInit, OnDestroy {
  ...
  constructor(private userService: UserService, private watchesService: WatchesService, private cartService: ShoppingCartService, private checkForInCartAlready: CheckForItemInCartAlreadyService) { }
  ...
  ngOnInit(): void {
    ...
    const watchesSubscription = this.watchesService.getWatches().subscribe(watchesObjs => {
      ...
      this.listItems = [...this.listItems, ...this.checkForInCartAlready.check([watchesObjs], this.cartItms)];
      // console.log(this.listItems);
    });
