...
export class GlovesComponent implements OnInit, OnDestroy {
  ...
  constructor(private userService: UserService, private glovesService: GlovesService, private cartService: ShoppingCartService, private checkForInCartAlready: CheckForItemInCartAlreadyService) { }
  ...
  ngOnInit(): void {
    ...
    const glovesSubscription = this.glovesService.getGloves().subscribe(glovesObjs => {
      ...
      this.listItems = [...this.listItems, ...this.checkForInCartAlready.check([glovesObjs], this.cartItms)];
      // console.log(this.listItems);
    });
