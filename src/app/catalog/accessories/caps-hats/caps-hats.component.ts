...
export class CapsHatsComponent implements OnInit, OnDestroy {
  ...
  constructor(private userService: UserService, private caps_hatsService: CapsHatsService, private cartService: ShoppingCartService, private checkForInCartAlready: CheckForItemInCartAlreadyService) { }
  ...
  ngOnInit(): void {
    ...
    const caps_hatsSubscription = this.caps_hatsService.getCapsHats().subscribe(caps_hatsObjs => {
      ...
      this.listItems = [...this.listItems, ...this.checkForInCartAlready.check([caps_hatsObjs], this.cartItms)];
      // console.log(this.listItems);
    });
