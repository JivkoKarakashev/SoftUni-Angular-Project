...
export class TrainersComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private fetchCatalogData(): Observable<Item[]> {
    this.loading = true;
    return this.trainersService.getCollectionSize()
      .pipe(
        catchError(err => {
          this.loading = false;
          this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
        ...
        switchMap(collSize => {
          ...
          ///////////////////////////////////////
          return this.trainersService.getTrainersByPage(this.paginationConfig.skipSizeReq, this.paginationConfig.pageSizeReq)
            .pipe(
              catchError(err => {
                this.loading = false;
                this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
