...
export class BootsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private fetchCatalogData(): Observable<Item[]> {
    this.loading = true;
    return this.bootsService.getCollectionSize()
      .pipe(
        catchError(err => {
          this.loading = false;
          this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
        ...
        switchMap(collSize => {
          ...
          ///////////////////////////////////////
          return this.bootsService.getBootsByPage(this.paginationConfig.skipSizeReq, this.paginationConfig.pageSizeReq)
            .pipe(
              catchError(err => {
                this.loading = false;
                this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
