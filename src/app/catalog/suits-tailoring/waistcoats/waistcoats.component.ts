...
export class WaistcoatsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private waistcoatsService: WaistcoatsService,
  ...
  ngOnInit(): void {
    ...
    const waistcoatsSubscription = this.userStateMgmnt.getUserState()
      .pipe(
        ...
        switchMap(itms => {
          ...
          return this.waistcoatsService.getWaistcoats();
        }),
        takeUntil(this.destroy$),
      ...
      .subscribe(
        {
          next: (wstctsObjs) => {
            this.loading = false;
            this.listItems = [...this.listItems, ...this.checkForInCartAlready.check([wstctsObjs], this.cartItms)];
    ...
    this.unsubscriptionArray.push(waistcoatsSubscription);
  }
