...
@Component({
  ...
  styleUrls: ['./workwear.component.css'],
  animations: [
...
export class WorkwearComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private fetchCatalogData(): Observable<Item[]> {
    this.loading = true;
    return this.workwearService.getCollectionSize()
      .pipe(
        catchError(err => {
          this.loading = false;
          this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
        ...
        switchMap(collSize => {
          ...
          ///////////////////////////////////////
          return this.workwearService.getWorkwearByPage(this.paginationConfig.skipSizeReq, this.paginationConfig.pageSizeReq)
            .pipe(
              catchError(err => {
                this.loading = false;
                this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
