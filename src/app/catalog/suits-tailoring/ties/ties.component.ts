...
export class TiesComponent implements OnInit, OnDestroy {
  ...
  constructor(private userService: UserService, private tiesService: TiesService, private cartService: ShoppingCartService, private checkForInCartAlready: CheckForItemInCartAlreadyService) { }
  ...
  ngOnInit(): void {
    ...
    const tiesSubscription = this.tiesService.getTies().subscribe(tiesObjs => {
      ...
      this.listItems = [...this.listItems, ...this.checkForInCartAlready.check([tiesObjs], this.cartItms)];
      // console.log(this.listItems);
    });
