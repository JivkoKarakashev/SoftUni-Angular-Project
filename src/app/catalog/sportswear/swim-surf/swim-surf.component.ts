...
export class SwimSurfComponent implements OnInit, OnDestroy {
  ...
  constructor(private userService: UserService, private swim_surfService: SwimSurfService, private cartService: ShoppingCartService, private checkForInCartAlready: CheckForItemInCartAlreadyService) { }
  ...
  ngOnInit(): void {
    ...
    const swim_surfSubscription = this.swim_surfService.getSwimSurf().subscribe(swim_surfObjs => {
      ...
      this.listItems = [...this.listItems, ...this.checkForInCartAlready.check([swim_surfObjs], this.cartItms)];
      // console.log(this.listItems);
    });
