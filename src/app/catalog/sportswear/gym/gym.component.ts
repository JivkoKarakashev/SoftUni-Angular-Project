...
export class GymComponent implements OnInit, OnDestroy {
  ...
  constructor(private userService: UserService, private gymService: GymService, private cartService: ShoppingCartService, private checkForInCartAlready: CheckForItemInCartAlreadyService) { }
  ...
  ngOnInit(): void {
    ...
    const gymSubscription = this.gymService.getGym().subscribe(gymObjs => {
      ...
      this.listItems = [...this.listItems, ...this.checkForInCartAlready.check([gymObjs], this.cartItms)];
      // console.log(this.listItems);
    });
