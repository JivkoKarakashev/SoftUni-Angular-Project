...
export class BottomsLeggingsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private fetchCatalogData(): Observable<Item[]> {
    this.loading = true;
    return this.bottoms_leggingsService.getCollectionSize()
      .pipe(
        catchError(err => {
          this.loading = false;
          this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
        ...
        switchMap(collSize => {
          ...
          ///////////////////////////////////////
          return this.bottoms_leggingsService.getBottomsLeggingsByPage(this.paginationConfig.skipSizeReq, this.paginationConfig.pageSizeReq)
            .pipe(
              catchError(err => {
                this.loading = false;
                this.errorsService.sethttpErrorsArrState([...this.httpErrorsArr, { ...err }]);
