...
export class BottomsLeggingsComponent implements OnInit, OnDestroy {
  ...
  constructor(private userService: UserService, private bottoms_leggingsService: BottomsLeggingsService, private cartService: ShoppingCartService, private checkForInCartAlready: CheckForItemInCartAlreadyService) { }
  ...
  ngOnInit(): void {
    ...
    const bottoms_leggingsSubscription = this.bottoms_leggingsService.getBottomsLeggings().subscribe(btms_lgingsObjs => {
      ...
      this.listItems = [...this.listItems, ...this.checkForInCartAlready.check([btms_lgingsObjs], this.cartItms)];
      // console.log(this.listItems);
    });
